...
void image::transSizeRender(HDC hdc, int destX, int destY,int Xsize,int Ysize)
{
	HDC h_dc = CreateCompatibleDC(_imageInfo->hMemDC);
	HBITMAP h_mem_bmp = CreateCompatibleBitmap(_imageInfo->hMemDC, WINSIZEX, WINSIZEY);
	HBITMAP oldObj = (HBITMAP)SelectObject(h_dc, h_mem_bmp);

	//트랜스 컬러 처리를 해야하냐
	if (_trans)
	{
		SetStretchBltMode(h_dc, STRETCH_HALFTONE);
		StretchBlt(h_dc, 0, 0, Xsize, Ysize, _imageInfo->hMemDC, 0, 0, _imageInfo->width, _imageInfo->height, SRCCOPY);
		//화면에 뿌려줄때 특정 픽셀값을 빼고 출력해준다
		GdiTransparentBlt(
			hdc,					//복사될 영역의 DC 
			destX,					//복사될 좌표 X
